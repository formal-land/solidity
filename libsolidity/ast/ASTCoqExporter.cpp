...
bool ASTCoqExporter::visit(SourceUnit const& _node)
{
	...
	std::string body = toCoq(_node.nodes());
...
bool ASTCoqExporter::visit(TryStatement const& _node)
{
	...
	std::string clauses = toCoq(_node.clauses());
...
bool ASTCoqExporter::visit(VariableDeclarationStatement const& _node)
{
	...
	std::string declarations = toCoq(_node.declarations());
	std::string initialValue = toCoqOrNull(_node.initialValue());

	m_currentValue = "VariableDeclarationStatement "s + varDecs + " "s + declarations + " "s + initialValue + "\n"s;
...
bool ASTCoqExporter::visit(FunctionCall const& _node)
{
	...
	std::string arguments = toCoq(_node.arguments());
...
bool ASTCoqExporter::visit(FunctionCallOptions const& _node)
{
	...
	std::string expression = toCoq(_node.expression());
	std::string options = toCoq(_node.options());
